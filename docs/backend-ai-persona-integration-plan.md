...
当前后端已经基于 Node.js + TypeScript + Express 实现了 `GET /api/search`，可以把知乎搜索结果映射为前端友好的 `items[]`。前端新增真实经历 Feed 后，单纯的内容召回结果已经不足以支撑页面，需要新增一层产品结构：

```text
analysis + feedItems + people + meta
```
...
- `cache_first`：优先缓存或 fixture；只有显式允许真实调用时才请求知乎 API。
- `replay`：只读本地知乎搜索 fixture，缺失时显式报错。
- `real`：优先 fixture/cache，缺失时在预算保护下真实请求知乎 API。
...
| `dataMode` | 标记 `mock/cache_first/replay/real` | 必须 |
| `features` | 前端能力开关 | 必须 |
| `analysis` | 问题理解和 loading 过程 | 必须 |
| `feedItems` | 真实经历 Feed 主列表 | 必须 |
| `paths` | 兼容空字段；不再用于前台分类导航 | 必须 |
| `people` | 前人样本主数据 | 必须 |
| `personas` | AI 分身快捷索引；可选，缺失时从 `people[].aiPersona` 派生 | 建议 |
| `sections` | 弱绑定 UI 扩展层；可选，缺失时前端按默认顺序渲染 | 建议 |
| `meta` | 来源、耗时、统计信息 | 建议 |
...
- `people[].pathId` 仅作内部兼容，前台卡片归属读取 `directionLabel`。
...
- 返回 `feedItems[] + people[]` 作为真实经历 Feed；`paths[]` 仅为空兼容字段。
