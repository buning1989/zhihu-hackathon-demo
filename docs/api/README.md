...
| `personas[]` | 可选 AI 分身快捷索引，只做入口导航，不是第二套人物数据；缺失时从 `people[].aiPersona` 派生。 |
| `sections[]` | 可选弱绑定布局辅助，可决定页面分区和排序；缺失时按默认顺序渲染。 |
...
- `personas[]`：渲染“可追问的经验回声”快捷入口。缺失时从 `people[].aiPersona` 派生；点击后用 `personId` 回查 `people[]`，不要从 `personas[]` 补全人物信息。
...
| `real` | 尝试真实知乎召回 | 调用知乎搜索 Provider 后进入产品层 composer；当前默认用 `real_rule_composer` 按公开内容关键词聚合路径、人物样本和 AI 分身入口，失败或无结果时降级 mock，`meta.fallbackUsed` 和 `debug.resolvedDataMode` 会说明。 |
