...
| `real` | 验证真实知乎召回组合 | 先调用底层搜索，再映射为产品层结构；失败或无结果默认返回错误，不再静默降级 mock。 | 只有请求显式传 `allowMockFallback: true` 才允许 mock 兜底。 |

无知乎 API Key、无 LLM Key 时，`mock` 必须完整可用。真实链路联调请使用 `dataMode: "real"`，让失败显性暴露；演示兜底才显式传 `allowMockFallback: true`。
