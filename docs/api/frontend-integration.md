...
| `personas[]` | AI 分身快捷索引。 | 渲染横向入口；点击后回查 `people[]`。 |
| `sections[]` | 页面分区辅助。 | 可决定模块顺序，不替代主数据。 |
...
- `provider: "zhihu"` 说明来自真实知乎召回，但仍只代表公开内容片段。
...
顶层 `personas[]` 用来做快捷入口，允许后端不返回。缺失时前端应从 `people[].aiPersona` 派生入口：
...
- 可用 `sections[]` 决定页面模块顺序和标题。
- `itemRefs[]` 只是引用 id，具体数据仍回到 `paths[]`、`people[]`、`personas[]`。
...
| `cache_first` | 验证未来缓存优先链路 | 当前仍使用内置 mock seed，`debug.notes` 标记 `cache_first currently uses bundled mock seed for demo continuity`。 | 可当成 mock 的兼容模式。 |
| `real` | 验证真实知乎召回组合 | 先调用底层搜索，再映射为产品层结构；失败或无结果会降级 mock。 | 看 `meta.fallbackUsed`、`debug.resolvedDataMode` 判断是否降级。 |

无知乎 API Key、无 LLM Key 时，`mock` 必须完整可用。`real` 模式失败不应该让 demo 主流程断掉。
...
| 400 | `DATA_MODE_INVALID` | `dataMode` 不是 `mock`、`cache_first`、`real`。 | 开发期报配置错误；生产可回退 `mock`。 |
...
底层兼容接口 `GET /api/search` 可能返回知乎鉴权或网络相关错误，例如 `ZHIHU_AUTH_FAILED`。前端 AI 分身页面不应直接依赖该接口；如需调试知乎召回，让后端同学使用它定位。
