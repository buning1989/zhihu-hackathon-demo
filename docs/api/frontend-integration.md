...
| `feedItems[]` | 真实经历 Feed 主列表。 | 渲染头像/昵称、来源、弱方向标签、片段、内容总结、查看原文和收藏。 |
| `paths[]` | 兼容字段。 | 主响应为空数组，不再渲染路径 tab、路径卡或分类导航。 |
| `people[]` | 人物样本主数据。 | 渲染人物详情、文章、匹配理由、AI 分身入口。 |
...
```json
{
  "id": "feed_person_city_pause",
  "personId": "person_city_pause",
  "authorName": "城市停靠样本",
  "authorAvatar": "",
  "sourceTitle": "关于不工作了能去哪儿的公开回答",
  "sourcePlatform": "知乎回答",
  ...
  "sourceRefs": ["source_mock_city_walk"],
  "saveSampleId": "person_city_pause"
}
```
...
- 如果 `evidenceIds/sourceRefs` 为空，隐藏该卡或降级为来源片段，不要补写经历。
...
- 点击后用 `personId` 回查 `people[]`，读取头像、来源、匹配理由、`directionLabel` 等完整数据。
...
- `itemRefs[]` 只是引用 id，具体数据仍回到 `feedItems[]`、`people[]` 和从 `people[].aiPersona` 派生的 persona 入口。
- 如果缺少 `sections[]` 或某个引用找不到，前端可按固定顺序渲染：`analysis -> feedItems/people -> personas`。
...
| `real` | 验证真实知乎召回组合 | 先命中本地 fixture/cache；缺失时才尝试真实知乎 API，并记录调用日志，不做本地每日预算拦截。失败或无结果默认返回错误。 | 只有请求显式传 `allowMockFallback: true` 才允许 mock 兜底。 |
...
- `feedItems[]` 存在：优先按 Feed 渲染；`paths[]` 即使存在也不再用于分类导航。
