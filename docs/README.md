...
2. [backend-ai-persona-integration-plan.md](./backend-ai-persona-integration-plan.md)：AI 分身产品层接口规划，当前 P0 主接口目标是 `POST /api/demo/search`。
3. [frontend-field-guide.md](./frontend-field-guide.md)：前端读取字段、fallback 规则和 AI 分身展示边界。
4. [backend-next-step-plan.md](./backend-next-step-plan.md)：既有 `GET /api/search` 链路说明，适合理解当前 Node.js 后端搜索能力。
5. [prompts/](./prompts/)：真实接入或调整 AI 分身前，阅读两份固定 system prompt。
