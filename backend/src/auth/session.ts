...
export const OAUTH_REDIRECT_URI_COOKIE_NAME = "zhihu_oauth_redirect_uri";
...
export function setOAuthRedirectUri(res: Response, redirectUri: string): void {
  if (!redirectUri) {
    clearCookie(res, OAUTH_REDIRECT_URI_COOKIE_NAME, { path: "/auth/zhihu/callback" });
    return;
  }

  setCookie(res, OAUTH_REDIRECT_URI_COOKIE_NAME, signCookieValue(redirectUri), {
    httpOnly: true,
    sameSite: "lax",
    secure: isProduction(),
    maxAgeSeconds: OAUTH_STATE_TTL_MS / 1000,
...
export function consumeOAuthRedirectUri(req: Request, res: Response): string {
  const redirectUri = readSignedCookie(req, OAUTH_REDIRECT_URI_COOKIE_NAME) || "";
  clearCookie(res, OAUTH_REDIRECT_URI_COOKIE_NAME, { path: "/auth/zhihu/callback" });
  return redirectUri;
}
