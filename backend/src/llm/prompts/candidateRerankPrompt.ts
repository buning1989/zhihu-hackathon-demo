...
- candidates：最多 5 条候选，每条只包含短摘要、短片段和规则粗排信号。

判断标准：
1. 优先选择和用户问题有真实关系的内容，而不是只含少量相同词的内容。
2. 优先保留有真实经历、具体选择、行动过程、结果反馈、代价、复盘、决策冲突的候选。
3. 降低泛泛观点、鸡汤、空泛建议、标题党、广告营销和弱相关内容优先级。
4. 避免大量同质内容；selected 目标数量 3-5 条，最多 5 条。
5. 不编造候选内容里没有的信息。
6. relationToUserIntent / summaryAngle / diversityKey / keepReason 都必须短句，单项不超过 24 个汉字。
7. dropped 可以为空；如需输出，最多 2 条，只写明显弱相关或明显营销候选。
